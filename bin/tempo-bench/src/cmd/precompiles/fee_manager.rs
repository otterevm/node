//! TIP Fee Manager precompile benchmarking implementation
...
use super::{
    BenchmarkScenario,
    contracts::IFeeManagerBenchmark,
    framework::{
...
pub struct FeeManagerBenchmarker<P: Provider> {
    base: BaseBenchmarker<P>,
}

impl<P: Provider> FeeManagerBenchmarker<P> {
    pub fn new(
        provider: Arc<P>,
...
impl<P: Provider + Send + Sync> PrecompileBenchmarker for FeeManagerBenchmarker<P> {
    async fn setup_state(&mut self) -> Result<()> {
        println!("Setting up Fee Manager benchmark state...");
        Ok(())
    }

    async fn deploy_contracts(&mut self) -> Result<()> {
        println!("Deploying Fee Manager benchmark contract...");
        self.base.contract_address = Some(Address::from([0x45; 20]));
        Ok(())
    }
    ...
    async fn generate_transactions(&self, ops_per_tx: u64) -> Result<Vec<TransactionRequest>> {
        ...
        let calldata = IFeeManagerBenchmark::spamSwapsCall {
    ...
    async fn run_benchmark(
        ...
        println!("Running Fee Manager benchmark - Scenario: {:?}", scenario);
        ...
        Ok(BenchmarkResult {
            precompile: "FeeManager".to_string(),
