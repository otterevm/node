...
The exchange maintains price‑time priority at each discrete price tick and supports auto‑replenishing “flip orders” that recreate themselves on the opposite side after being fully filled. 
...
Within each price tick, orders are appended in their creation order, maintaining FIFO price‑time priority.
...
Places a limit order against the pair of `token` and its quote. Escrows funds: bids escrow quote at tick price; asks escrow base. Orders respect price‑time priority.
...
Like `place`, but marks the order as a flip order. When fully filled, a new order for the same maker is created on the opposite side at `flipTick` (which must be greater than `tick` for bids and less for asks) and inserted onto the active book.
...
Cancels an order owned by the caller. The order is removed from the tick queue, liquidity is decremented, and remaining escrow is refunded to the caller’s internal balance.
...
Monotonic counters for the last processed active order and latest order identifier.
...
Notable revert reasons (string reverts in the reference implementation):
