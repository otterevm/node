...
- **Expiry**: Unix timestamp when the key becomes invalid (use `u64::MAX` for never expires)
...
interface IAccountKeychain {
     ...
     * @param expiry Unix timestamp when key expires (must be in the future; use type(uint64).max for never expires)
...
- `expiry` MUST be greater than the current block timestamp (use `type(uint64).max` for never expires; reverts with `ExpiryInPast`)
...
- Use `u64::MAX` (`type(uint64).max` in Solidity) for keys that should never expire
- Expiry is checked as: `current_timestamp >= expiry` (key is expired when current time reaches or exceeds expiry)
- Authorizing keys with `expiry <= current_timestamp` is rejected with `ExpiryInPast` error
