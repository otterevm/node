...
interface ITIP20 {
    ...
    /// @notice Emitted when rewards are distributed to opted-in token holders
...
- When `paused` is `true`, no functions that move tokens (`transfer`, `transferFrom`, memo variants, `systemTransferFrom`, `distributeReward`, `setRewardRecipient`, `claimRewards`) can succeed.
