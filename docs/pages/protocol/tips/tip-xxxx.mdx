...
|------|-------|-------------|
| 0 (bits 0-159) | `owner` | Owner address (160 bits) |
...
The `initialized` bit occupies the highest bit of slot 0, which is unused by the
`owner` address (addresses are 160 bits, leaving 96 bits available).
...
2. **Lazy initialization**: On first read, V2 automatically populates itself
   from V1 state
...
The following view functions trigger initialization if `initialized == false`:
...
22. **First read triggers init**: Calling `getValidators()` on uninitialized V2
    populates from V1
23. **All triggering functions**: Each of `getValidators`, `getActiveValidators`,
    `validatorCount`, `validatorByIndex`, `validatorByAddress`,
    `validatorByPublicKey` triggers initialization
24. **Initialization is idempotent**: Subsequent reads do not re-initialize
25. **Active validators only**: V1 validators with `active == false` are not
    imported
26. **Owner copied**: V2 `owner()` returns the same value as V1 `owner()`
...
29. **Initialized bit set**: After initialization, bit 255 of slot 0 is set
