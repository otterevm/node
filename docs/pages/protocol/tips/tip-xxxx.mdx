...
By recording `addedAtHeight`, `deletedAtHeight`, and `deletedBeforeMigration`, nodes can determine DKG players for any epoch using only current state. When preparing for a DKG ceremony in epoch `E+2`, a node reads the contract at `boundary(E)` and filters:

```
players(E+2) = validators.filter(v =>
    !v.deletedBeforeMigration &&
...
interface IValidatorConfigV2 {
    ...
    /// @param addedAtHeight Block height when validator was added (0 for validators migrated from V1)
    /// @param deletedAtHeight Block height when validator was deleted (0 = active)
    /// @param deletedBeforeMigration True if validator was inactive in V1 before migration (packed with uint64s)
    struct Validator {
        bytes32 publicKey;
    ...
    /// @notice Get only active validators (where deletedAtHeight == 0 && deletedBeforeMigration == false)
...
| Aspect | V1 | V2 |
|--------|----|----|
| Status field | `active: bool` | `deletedAtHeight: uint64` + `deletedBeforeMigration: bool` |
...
| Historical queries | Requires historical state | Filter `getValidators()` by `addedAtHeight`/`deletedAtHeight`/`deletedBeforeMigration` |
...
2. **Immutable validators**: Once a validator is added, its `publicKey`,
   `validatorAddress`, `inboundAddress`, `outboundAddress`, `index`,
   `addedAtHeight`, and `deletedBeforeMigration` fields never change.
...
3. **Delete-once**: A validator's `deletedAtHeight` can only transition from 0
   ...
   Similarly, `deletedBeforeMigration` is immutable once set.
...
8. **Historical consistency**: For any height H, `getValidatorSetAtHeight(H)`
   returns exactly the validators where `!deletedBeforeMigration &&
...
**Conversion from V1 to V2 format** (for reads only, no state changes):
...
| `index` | `index` |
| N/A | `addedAtHeight = 0` (unknown for V1 validators) |
| `active == true` | `deletedAtHeight = 0`, `deletedBeforeMigration = false` |
| `active == false` | `deletedAtHeight = 0`, `deletedBeforeMigration = true` |
...
When any of these functions is called and `initialized == false`:
...
2. For each V1 validator (both active and inactive):
   - Create a V2 validator entry with:
     ...
     - `deletedAtHeight`: set to 0 (for both active and inactive V1 validators)
     - `deletedBeforeMigration`: if V1 `active == true`, set to false; if V1 `active == false`, set to true
...
5. Proceed with the original write operation
...
- **All validators imported**: Both active and inactive V1 validators are
    imported; inactive ones have `deletedBeforeMigration = true`
- **No genesis setup required**: No need to set `validatorsArray` at genesis;
    reads proxy through V1 until first write
...
27. **Filter logic**: Caller can correctly filter by
    `!deletedBeforeMigration && addedAtHeight <= H && (deletedAtHeight == 0 || deletedAtHeight > H)`
...
33. **V1 read failure**: If V1 read fails, initialization sets bit without
    importing validators
34. **V1 to V2 format conversion**: Reads before init correctly convert V1 format
    (`active` bool) to V2 format (`deletedBeforeMigration`)
