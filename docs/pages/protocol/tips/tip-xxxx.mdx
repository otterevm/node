...
///      - `active` bool replaced by `addedAtHeight`, `deletedAtHeight`, and `deletedBeforeMigration`
...
2. For each V1 validator (both active and inactive):
   - Create a V2 validator entry with:
     ...
     - `deletedAtHeight`: set to 0
     - `deletedBeforeMigration`: set to true if V1 `active == false`, otherwise false
...
7. **getActiveValidators**: Returns only validators with `deletedAtHeight == 0 && deletedBeforeMigration == false`
