...
* If the user's fee token differs from the validator's preferred token, execute the fee swap immediately via the [fee AMM](/protocol/fees/spec-fee-amm). If there is insufficient liquidity, the transaction is invalid.
...
When a user submits a transaction on Tempo, fees are paid in their chosen stablecoin (determined by the [fee token preferences](#fee-token-preferences) hierarchy). If the validator prefers a different stablecoin, the Fee AMM converts the user's payment to the validator's preferred token.

#### 1. User submits transaction

The transaction is submitted with the fee token determined by the preference hierarchy.

#### 2. Pre-transaction collection and conversion

Before the transaction executes, the `FeeManager` contract collects the maximum possible fee amount from the user and converts it if needed:
...
- If the user's token differs from the validator's preferred token:
  - Checks if the Fee AMM has enough liquidity
  - Executes the fee swap at the fixed rate of **0.9970**
  - Adds the converted amount to the validator's fee balance
  - Adds the valiator's address to a tracking array, for distribution at end of block
- If tokens match, adds the fee directly to the validator's fee balance

If any check fails (insufficient balance or insufficient liquidity), the transaction is rejected before execution.
...
#### 4. End-of-block distribution

At the end of each block, the protocol:

1. Calls `executeBlock()` on the FeeManager
2. Transfers each validator's accumulated fee balance to their account
3. Clears fee tracking arrays

This step batches the distribution of fees to validators, avoiding the need for extra Transfer events every transaction.
...
5. The Fee AMM swaps 1.0 USDC → 0.9970 USDT (1.0 × 0.9970)
6. 0.9970 USDT is added to the validator's fee balance
7. Transaction executes and uses 0.8 USDC worth of gas (for accounting only)
8. No refund is issued
9. At block end, the validator receives their accumulated 0.9970 USDT
...
- **Pre-transaction**: ~5,000 gas for balance checks, fee swap execution, and fee accounting
