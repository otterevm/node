...
Executes a fee swap immediately, converting `userToken` to `validatorToken` at the fixed rate m = 0.9970. Called by `FeeManager.collectFeePreTx` during pre-transaction fee collection. Calculates `amountOut = (amountIn * M) / SCALE`. Reverts if insufficient liquidity. Updates reserves immediately. Access: Internal only (called by FeeManager).
...
The `FeeManager` is a singleton contract that implements all the functions of the Fee AMM for every pool. It also handles the collection and accounting of fees during each transaction, stores fee token preferences for users and validators, maintains fee balances per validator, and implements the `executeBlock()` function that transfers accumulated fees to validators at the end of each block.
...
Called by the protocol before transaction execution. The fee token (`userToken`) is determined by the protocol before calling using logic that considers: explicit tx fee token, setUserToken calls, stored user preference, tx.to if TIP-20. Collects the maximum fee from the user using `transferFeePreTx`. If user token differs from validator token, executes the fee swap immediately via `executeFeeSwap` and adds the output to the validator's fee balance. If tokens match, adds `maxAmount` directly to the validator's fee balance. Adds the validator's address to a tracking array for distribution at end of block. Access: Protocol only (`msg.sender == address(0)`).

```solidity
function executeBlock() external
```

Called once in a system transaction at the end of each block. Transfers accumulated fee balances to all validators who received fees during the block. Clears fee tracking arrays. Access: Protocol only (`msg.sender == address(0)`).
...
1. **Pre-Transaction**:
   ...
   - `FeeManager.collectFeePreTx(user, userToken, maxAmount)` is called:
     ...
     - If user token differs from validator token, executes fee swap immediately via `executeFeeSwap()`
...
2. **Block End**:
   - System transaction calls `FeeManager.executeBlock()`:
     - Transfers accumulated fee balances to each validator
...
Fee swaps are designed to be gas-free from the user perspective. The pre-tx step in each transaction does not cost any gas; nor does the system transaction at the end of each block.
