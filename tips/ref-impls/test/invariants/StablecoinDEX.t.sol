...
contract StablecoinDEXInvariantTest is InvariantBaseTest {
    ...
    /// @dev Verifies TEMPO-DEX10 (balance solvency), TEMPO-DEX11/11 (tick consistency), TEMPO-DEX12/9 (best tick)
    function invariantStablecoinDEX() public view {
        // TEMPO-DEX10: Check pathUSD balance
        ...
        // TEMPO-DEX10: Check each base token balance
        for (uint256 t = 0; t < _tokens.length; t++) {
        ...
        // TEMPO-DEX12 & TEMPO-DEX13: Best bid/ask tick consistency for all tokens
        for (uint256 t = 0; t < _tokens.length; t++) {
            _assertBestTickConsistency(address(_tokens[t]));
        }

        // TEMPO-DEX11 & TEMPO-DEX15: Tick level and bitmap consistency for all tokens
        for (uint256 t = 0; t < _tokens.length; t++) {
    ...
    function _swapExactAmountIn(
        ...
        try exchange.swapExactAmountIn(
            ...
            // TEMPO-DEX8: Each swap can increase dust by at most 1 per order filled
            uint256 dustAfterSwap = _computeDust();
            assertLe(
                dustAfterSwap,
                _dustBeforeSwap + ordersFilled,
                "TEMPO-DEX8: swap increased dust by more than 1 per order filled"
            );
