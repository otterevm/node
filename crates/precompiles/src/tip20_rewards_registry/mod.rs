...
fn calc_stream_key(token: Address, end_time: u128) -> B256 {
    keccak256((token, end_time).abi_encode())
}
...
impl TIP20RewardsRegistry {
    ...
    pub fn add_stream(&mut self, token: Address, end_time: u128) -> Result<()> {
        let stream_key = calc_stream_key(token, end_time);
        let mut stream_ending_at = self.ending_streams.at(end_time);
        let length = stream_ending_at.len().map(|len| U256::from(len))?;

        self.stream_index.at(stream_key).write(length)?;
    ...
    pub fn remove_stream(&mut self, token: Address, end_time: u128) -> Result<()> {
        let stream_key = calc_stream_key(token, end_time);
        let index = self.stream_index.at(stream_key).read()?.to::<usize>();
        ...
        if index != last_index {
            ...
            let last_stream_key = calc_stream_key(last_token, end_time);
            self.stream_index
                .at(last_stream_key)
    ...
    pub fn finalize_streams(&mut self, sender: Address) -> Result<()> {
        ...
        while current_timestamp >= next_timestamp {
            ...
            for token in tokens {
                ...
                let stream_key = calc_stream_key(token, next_timestamp);
                self.stream_index.at(stream_key).delete()?;
            }
...
mod tests {
    ...
    use crate::{
        ...
        storage::{ContractStorage, PrecompileStorageContext, hashmap::HashMapStorageProvider},
    ...
    #[test]
    fn test_read_streams_ending_at() -> eyre::Result<()> {
        let (mut storage, _admin) = setup_registry(1000);
        let _guard = storage.enter().unwrap();
        let mut registry = TIP20RewardsRegistry::new();
        registry.initialize()?;
