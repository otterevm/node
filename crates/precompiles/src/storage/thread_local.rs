...
/// Thread-local storage accessor that implements `PrecompileStorageProvider`.
///
/// This type provides access to the current thread-local storage context.
/// It must only be used while a `StorageGuard` is active.
///
/// # Sync with `PrecompileStorageProvider`
///
/// This type mirrors `PrecompileStorageProvider` methods but with split mutability:
/// - Read operations (`sload`, `tload`, etc.) take `&self`
/// - Write operations (`sstore`, `tstore`, `emit_event`, etc.) take `&mut self`
