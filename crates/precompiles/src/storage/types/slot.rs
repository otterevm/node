...
impl<T> Slot<T> {
    ...
    #[inline]
    pub fn new(slot: U256, address: Rc<Address>) -> Self {
        Self {
            slot,
            ctx: LayoutCtx::FULL,
            address,
    ...
    #[inline]
    pub fn new_with_ctx(slot: U256, ctx: LayoutCtx, address: Rc<Address>) -> Self {
        Self {
            slot,
            ctx,
            address,
    ...
    /// Creates a full-slot accessor. For packed fields, use `new_at_loc` instead.
    #[inline]
    pub fn new_at_offset(base_slot: U256, offset_slots: usize, address: Rc<Address>) -> Self {
        Self {
...
mod tests {
    ...
    #[test]
    fn test_slot_size() {
        // slot (U256) 32 bytes + LayoutCtx (usize) 8 bytes + Rc<Address> 8 bytes
