...
use crate::storage::{StorageOps, StorageType};
...
impl<T, const SLOT: [u64; 4]> Slot<T, SLOT> {
    ...
    /// ```
    #[inline]
    pub fn read<S: StorageOps>(storage: &mut S) -> T
    where
        T: StorageType,
    {
        let value = storage
        ...
        T::from_u256(value).expect("TODO: handle error")
    }
    ...
    /// ```
    #[inline]
    pub fn write<S: StorageOps>(storage: &mut S, value: T)
    where
        T: StorageType,
    {
        storage
            .sstore(Self::slot(), value.to_u256())
            .expect("TODO: handle error")
    }
}
...
mod tests {
    ...
    fn test_slot_read_write_u256() {
        ...
        let mut contract = TestContract { address: addr, storage: &mut storage };

        type TestSlot = Slot<U256, { [42, 0, 0, 0] }>;
        let test_value = U256::from(12345u64);

        // Write using new API
        TestSlot::write(&mut contract, test_value);
    ...
    fn test_slot_read_write_address() {
        ...
        // Write
        OwnerSlot::write(&mut contract, test_addr);

        // Read
        let loaded = OwnerSlot::read(&mut contract);
    ...
    fn test_slot_read_write_bool() {
        ...
        // Write true
        PausedSlot::write(&mut contract, true);
        assert!(PausedSlot::read(&mut contract));

        // Write false
        PausedSlot::write(&mut contract, false);
        assert!(!PausedSlot::read(&mut contract));
    }
    ...
    fn test_slot_read_write_string() {
        ...
        NamedSlot::write(&mut contract, test_name.to_string());

        let loaded = NamedSlot::read(&mut contract);
    ...
    fn test_slot_default_value_is_zero() {
        ...
        let value = UninitializedSlot::read(&mut contract);
    ...
    fn test_slot_overwrite() {
        ...
        // Write initial value
        CounterSlot::write(&mut contract, 100);
        assert_eq!(CounterSlot::read(&mut contract), 100);

        // Overwrite with new value
        CounterSlot::write(&mut contract, 200);
        assert_eq!(CounterSlot::read(&mut contract), 200);
    }
}
