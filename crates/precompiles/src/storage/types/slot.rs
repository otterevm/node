...
impl<T> Slot<T> {
    ...
    /// By default, creates a full-slot accessor. Chain with `.packed(offset)` for packed fields.
    #[inline]
    ...
    /// By default, creates a full-slot accessor. Chain with `.packed(offset_bytes)` for packed fields.
    #[inline]
    ...
    /// Configures this slot for packed access at the given byte offset.
    ///
    /// This is a builder method that modifies the layout context to support
    /// read-modify-write operations for packed fields.
    ///
    /// # Example
    ///
    /// ```ignore
    /// let packed_slot = Slot::<u64>::new_at_offset(base, 0).packed(8);
    /// packed_slot.write(&mut contract, value)?;
    /// ```
    #[inline]
    pub const fn packed(mut self, offset: usize) -> Self {
        self.ctx = crate::storage::types::LayoutCtx::Packed(offset);
        self
    }
