...
impl<K, V, const SLOT: [u64; 4]> Mapping<K, V, SLOT> {
    ...
    /// ```
    #[inline]
    pub fn read<S: StorageOps>(storage: &mut S, key: K) -> V
    where
        K: StorageKey,
        V: StorageType,
    {
        let slot = mapping_slot(key.as_storage_bytes(), Self::slot());
        let value = storage.sload(slot).expect("TODO: handle error");
        V::from_u256(value).expect("TODO: handle error")
    }
    ...
    /// ```
    #[inline]
    pub fn write<S: StorageOps>(storage: &mut S, key: K, value: V)
    where
        K: StorageKey,
        V: StorageType,
    {
    ...
    Mapping<K1, Mapping<K2, V, DUMMY>, SLOT>
{
    ...
    /// ```
    #[inline]
    pub fn read_nested<S: StorageOps>(
        storage: &mut S,
        key1: K1,
        key2: K2,
    ) -> V
...
mod tests {
    ...
    impl<'a, S: PrecompileStorageProvider> StorageOps for TestContract<'a, S> {
        fn sstore(&mut self, slot: U256, value: U256) -> Result<(), revm::precompile::PrecompileError> {
            self.storage.sstore(self.address, slot, value)
        }

        fn sload(&mut self, slot: U256) -> Result<U256, revm::precompile::PrecompileError> {
    ...
    fn test_mapping_read_write_balances() {
        ...
        let mut contract = TestContract { address: token_addr, storage: &mut storage };
        ...
        // Write balances
        NamedMapping::write(&mut contract, user1, balance1);
        NamedMapping::write(&mut contract, user2, balance2);

        // Read balances
        let loaded1 = NamedMapping::read(&mut contract, user1);
        let loaded2 = NamedMapping::read(&mut contract, user2);
    ...
    fn test_mapping_read_default_is_zero() {
        ...
        let balance = NamedMapping::read(&mut contract, user);
    ...
    fn test_mapping_overwrite() {
        ...
        NamedMapping::write(&mut contract, user, U256::from(100));
        assert_eq!(
            NamedMapping::read(&mut contract, user),
            U256::from(100)
        );

        // Overwrite with new balance
        NamedMapping::write(&mut contract, user, U256::from(200));
        assert_eq!(
            NamedMapping::read(&mut contract, user),
            U256::from(200)
        );
    ...
    fn test_nested_mapping_read_write_allowances() {
        ...
        NestedMapping::write_nested(&mut contract, owner, spender1, allowance1);
        NestedMapping::write_nested(&mut contract, owner, spender2, allowance2);

        // Read allowances using nested API
        let loaded1 = NestedMapping::read_nested(&mut contract, owner, spender1);
        let loaded2 = NestedMapping::read_nested(&mut contract, owner, spender2);
    ...
    fn test_nested_mapping_default_is_zero() {
        ...
        let allowance = NestedMapping::read_nested(&mut contract, owner, spender);
    ...
    fn test_nested_mapping_independence() {
        ...
        NestedMapping::write_nested(&mut contract, owner1, spender, U256::from(100));

        // Verify owner2 -> spender is still zero (independent slot)
        let allowance2 = NestedMapping::read_nested(&mut contract, owner2, spender);
        assert_eq!(allowance2, U256::ZERO);

        // Verify owner1 -> spender is unchanged
        let allowance1 = NestedMapping::read_nested(&mut contract, owner1, spender);
    ...
    fn test_mapping_with_different_key_types() {
        ...
        let mut contract = TestContract { address: contract_addr, storage: &mut storage };
        ...
        NoncesMapping::write(&mut contract, user, nonce);
        let loaded_nonce = NoncesMapping::read(&mut contract, user);
        assert_eq!(loaded_nonce, nonce);

        // Mapping with bool value
        type FlagsMapping = Mapping<Address, bool, { [13, 0, 0, 0] }>;
        FlagsMapping::write(&mut contract, user, true);
        let loaded_flag = FlagsMapping::read(&mut contract, user);
        assert!(loaded_flag);
    }
}
