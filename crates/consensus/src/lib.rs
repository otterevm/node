...
mod tests {
    ...
    fn test_validate_header_shared_gas_mismatch() {
        ...
        assert!(
            matches!(&err, ConsensusError::Other(msg) if msg == "Shared gas limit does not match header gas limit")
        );
    }
    ...
    fn test_validate_header_non_payment_gas_mismatch() {
        ...
        let err = result.unwrap_err();
        assert!(
            matches!(&err, ConsensusError::Other(msg) if msg == "Non-payment gas limit does not match header gas limit")
        );
    }
    ...
    fn test_validate_header_timestamp_milli_gte_1000() {
        ...
        let err = result.unwrap_err();
        assert!(
            matches!(&err, ConsensusError::Other(msg) if msg == "Timestamp milliseconds part must be less than 1000")
        );
    ...
    fn test_validate_header_against_parent_timestamp_not_increasing() {
        ...
        let err = result.unwrap_err();
        assert!(
            matches!(&err, ConsensusError::TimestampIsInPast { parent_timestamp, timestamp }
                if *parent_timestamp == parent_timestamp_millis && *timestamp == child_timestamp_millis)
        );
    }
    ...
    fn test_validate_block_pre_execution_invalid_system_tx() {
        ...
        let err = result.unwrap_err();
        let expected_msg = format!("Invalid system transaction: {tx_hash}");
        assert!(matches!(&err, ConsensusError::Other(msg) if msg == &expected_msg));
    }
    ...
    fn test_validate_block_pre_execution_no_system_tx() {
        ...
        let err = result.unwrap_err();
        assert!(
            matches!(&err, ConsensusError::Other(msg) if msg == "Block must contain end-of-block system txs")
        );
    }
    ...
    fn test_validate_block_pre_execution_system_tx_out_of_order() {
        ...
        let err = result.unwrap_err();
        assert!(
            matches!(&err, ConsensusError::Other(msg) if msg == "Invalid end-of-block system tx order")
        );
    }
}
