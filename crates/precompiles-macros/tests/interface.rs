...
#[contract(ITestToken, TestTokenError)]
...
impl<S: storage::PrecompileStorageProvider> TestTokenCall for TestToken<'_, S> {
    ...
    ) -> Result<bool, TestTokenError> {
        let balance = self._get_balances(*s);
        if call.amount > balance {
            return Err(TestTokenError::InsufficientBalance(
                ITestToken::InsufficientBalance {
    ...
    fn mint(&mut self, _s: &Address, call: ITestToken::mintCall) -> Result<(), TestTokenError> {
        let balance = self._get_balances(call.to);
        self._set_balances(call.to, balance + call.amount);
        Ok(())
    }

    fn burn(&mut self, s: &Address, call: ITestToken::burnCall) -> Result<(), TestTokenError> {
        let balance = self._get_balances(*s);
        if call.amount > balance {
            return Err(TestTokenError::InsufficientBalance(
                ITestToken::InsufficientBalance {
...
fn test_contract_without_interface() {
    ...
    simple._set_value(U256::from(42));
    simple._set_name("Test".to_string());
    simple._set_counter(100);

    assert_eq!(simple._get_value(), U256::from(42));
    assert_eq!(simple._get_name(), "Test");
    assert_eq!(simple._get_counter(), 100);
...
fn test_contract_with_map_attribute() {
    ...
    token._set_balances(account1, U256::from(1000));
    token._set_balances(account2, U256::from(500));

    assert_eq!(token._get_balances(account1), U256::from(1000));
    assert_eq!(token._get_balances(account2), U256::from(500));

    // Test nested mapping
    let owner = test_address(4);
    let spender = test_address(5);

    token._set_allowances(owner, spender, U256::from(200));
    assert_eq!(token._get_allowances(owner, spender), U256::from(200));
}
...
fn test_multiple_storage_types() {
    ...
    // Set all types
    multi._set_string_val("Hello".to_string());
    multi._set_u256_val(U256::from(12345));
    multi._set_u64_val(999);
    multi._set_bool_val(true);
    multi._set_address_val(test_address(99));

    let key = test_address(10);
    multi._set_mapping_val(key, U256::from(777));

    // Verify all types
    assert_eq!(multi._get_string_val(), "Hello");
    assert_eq!(multi._get_u256_val(), U256::from(12345));
    assert_eq!(multi._get_u64_val(), 999);
    assert!(multi._get_bool_val());
    assert_eq!(multi._get_address_val(), test_address(99));
    assert_eq!(multi._get_mapping_val(key), U256::from(777));
}
...
fn test_nested_mapping_storage() {
    ...
    nested._set_nested(key1, key2, U256::from(888));
    assert_eq!(nested._get_nested(key1, key2), U256::from(888));

    // Test multiple nested entries
    let key3 = test_address(4);
    nested._set_nested(key1, key3, U256::from(999));
    assert_eq!(nested._get_nested(key1, key3), U256::from(999));
...
fn test_contract_with_generic_storage_provider() {
    ...
    contract1._set_value(U256::from(42));
    assert_eq!(contract1._get_value(), U256::from(42));
...
fn test_dispatcher_metadata_functions() {
    ...
    // Setup
    token._set_name("Test Token".to_string());
    token._set_symbol("TEST".to_string());
...
fn test_dispatcher_view_functions() {
    ...
    token._set_balances(account, U256::from(1000));
    token._set_allowances(account, spender, U256::from(500));
...
fn test_dispatcher_mutate_functions() {
    ...
    token._set_balances(sender, U256::from(1000));
    ...
    assert_eq!(token._get_balances(sender), U256::from(900));
    assert_eq!(token._get_balances(recipient), U256::from(100));
    ...
    let success = bool::abi_decode(&result.bytes).unwrap();
    assert!(success);
    assert_eq!(token._get_allowances(sender, spender), U256::from(200));
}
...
fn test_dispatcher_mutate_void_functions() {
    ...
    assert_eq!(token._get_balances(recipient), U256::from(500));

    // Setup for burn
    token._set_balances(sender, U256::from(300));
    ...
    assert_eq!(token._get_balances(sender), U256::from(200));
}
...
fn test_dispatcher_error_handling() {
    ...
    token._set_balances(sender, U256::from(50));
    let calldata = ITestToken::transferCall {
        to: recipient,
        amount: U256::from(100),
    }
    ...
    assert!(result.reverted);
    assert_eq!(result.gas_used, MUTATE_FUNC_GAS);
    let error = TestTokenError::abi_decode(&result.bytes).unwrap();
    assert!(matches!(error, TestTokenError::InsufficientBalance(_)));
...
fn test_dispatcher_selector_routing() {
    ...
    token._set_name("Test".to_string());
    ...
    token._set_symbol("TST".to_string());
    ...
    token._set_balances(account, U256::from(999));
...
fn test_dispatcher_all_function_types_together() {
    ...
    // Setup metadata
    token._set_name("Full Test".to_string());
    token._set_symbol("FULL".to_string());
