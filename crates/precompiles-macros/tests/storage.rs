...
fn test_mixed_slot_allocation() {
    ...
    // Set all fields
    mixed._set_field_a(U256::from(100));
    mixed._set_field_b(U256::from(200));
    mixed._set_field_c(U256::from(300));
    mixed._set_field_d(U256::from(400));

    // Verify values
    assert_eq!(mixed._get_field_a(), U256::from(100));
    assert_eq!(mixed._get_field_b(), U256::from(200));
    assert_eq!(mixed._get_field_c(), U256::from(300));
    assert_eq!(mixed._get_field_d(), U256::from(400));
...
fn test_string_storage() {
    ...
    str_storage._set_short_string(test_str.clone());
    assert_eq!(str_storage._get_short_string(), test_str);

    // Test empty string
    str_storage._set_another_string(String::new());
    assert_eq!(str_storage._get_another_string(), "");

    // Test max length (31 bytes)
    let max_str = "a".repeat(31);
    str_storage._set_short_string(max_str.clone());
    assert_eq!(str_storage._get_short_string(), max_str);
}
...
fn test_default_values() {
    ...
    assert_eq!(defaults._get_counter(), 0);
    assert!(!defaults._get_flag());
    assert_eq!(defaults._get_amount(), U256::ZERO);
}
