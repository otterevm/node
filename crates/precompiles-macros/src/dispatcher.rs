...
//!
//! This module generates the `impl Precompile for ContractType` block that routes
...
pub(crate) fn gen_dispatcher(
    ...
    let match_arms: Vec<TokenStream> = match_results
        .iter()
        .map(|result| gen_match_arm(&trait_name, interface_type, error_type, result))
        .collect();
...
fn gen_match_arm(
    ...
    match func.kind() {
        ...
        FunctionKind::Mutate => {
            quote! {
                #call_type::SELECTOR => {
                    crate::mutate::<#call_type, #error_type>(
                        calldata,
                        msg_sender,
        ...
        FunctionKind::MutateVoid => {
            quote! {
                #call_type::SELECTOR => {
                    crate::mutate_void::<#call_type, #error_type>(
                        calldata,
                        msg_sender,
                        |s, call| #trait_name::#method_name(self, s, call)
                    )
