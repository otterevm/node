...
impl syn::parse::Parse for ContractAttrs {
    fn parse(input: syn::parse::ParseStream<'_>) -> syn::Result<Self> {
        ...
        let error_type = if input.peek(syn::Token![,]) {
...
fn gen_contract_output(
    ...
    let impl_output = if let Some(ref interface) = attrs.interface_type {
        gen_contract_impl(&ident, interface, attrs.error_type.as_ref(), &fields)?
    } else {
        quote! {}
    };
...
fn gen_contract_impl(
    ...
    let trait_output = traits::gen_trait_and_impl(ident, interface, error_type, &getters);

    let dispatcher_output = if let Some(error_ty) = error_type {
        dispatcher::gen_dispatcher(ident, interface, error_ty, &getters)
    } else {
        quote! {}
    };
