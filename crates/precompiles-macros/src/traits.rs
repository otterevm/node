...
pub(crate) fn gen_trait_and_impl<'a>(
    ...
    let call_trait = gen_call_trait(&c_name, &s_name, error_type, match_results);
...
fn gen_storage_method_sig_or_impl(gen_sig: bool, result: &GetterFn<'_>) -> TokenStream {
    ...
    match &result.field_match {
        GetterInfo::Direct { field } => {
            ...
            if gen_sig {
                quote! { fn #getter_name(&mut self) -> #return_type; }
            } else {
                quote! {
                    fn #getter_name(&mut self) -> #return_type {
                        self.#getter_name()
                    }
                }
            }
        ...
        GetterInfo::Mapping { field, key_param } => {
            ...
            if gen_sig {
                quote! { fn #getter_name(&mut self, #key: #ty) -> #return_type; }
            } else {
                quote! {
                    fn #getter_name(&mut self, #key: #ty) -> #return_type {
        ...
        GetterInfo::NestedMapping {
            ...
            if gen_sig {
                quote! { fn #getter_name(&mut self, #key1: #ty1, #key2: #ty2) -> #return_type; }
            } else {
                quote! {
                    fn #getter_name(&mut self, #key1: #ty1, #key2: #ty2) -> #return_type {
...
fn gen_call_trait(
    ...
    let interface_methods: Vec<TokenStream> = match_results
        .iter()
        .map(|result| gen_call_trait_method(result, error_type))
        .collect();
...
fn gen_call_trait_method(result: &GetterFn<'_>, error_type: Option<&Type>) -> TokenStream {
    if let GetterInfo::NoMatch = &result.field_match {
        return gen_call_sig(result, error_type);
    }
    ...
    match func.is_view {
        true if has_params => quote! {
            fn #method_name(&mut self, call: #call_type) -> #return_type {
                #body
            }
        },
        true => quote! {
        ...
        false => {
            if let Some(error_ty) = error_type {
                quote! {
                    fn #method_name(&mut self, msg_sender: &::alloy::primitives::Address, call: #call_type) -> Result<#return_type, #error_ty> {
                        #body
                    }
                }
            } else {
                quote! {
                    fn #method_name(&mut self, msg_sender: &::alloy::primitives::Address, call: #call_type) -> #return_type {
                        #body
                    }
                }
            }
...
fn gen_call_sig(result: &GetterFn<'_>, error_type: Option<&Type>) -> TokenStream {
    let func = &result.function;
    let method_name = format_ident!("{}", func.name);
    let return_type = &func.return_type;
    let call_type = &func.call_type_path;
...
mod tests_trait {
    ...
    fn test_generate_trait_with_direct_match() {
        ...
        let trait_code = gen_trait_and_impl(&struct_name, &interface_type, None, &matches);
