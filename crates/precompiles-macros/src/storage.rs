...
fn classify_field(ty: &Type) -> syn::Result<FieldKind<'_>> {
    // Try to parse as Mapping<K, V>
    if let Some((key_ty, value_ty)) = extract_mapping_types(ty) {
        // Check if value is also a Mapping (nested mapping)
...
/// Generate the ContractStorage trait implementation
    ...
    match &allocated.kind {
        FieldKind::Direct => {
            ...
            quote! {
                impl<'a, S: crate::storage::PrecompileStorageProvider> #struct_name<'a, S> {
                    #[inline]
                    fn #getter_name(&mut self) -> #field_ty {
        ...
        FieldKind::Mapping {
            ...
            quote! {
                impl<'a, S: crate::storage::PrecompileStorageProvider> #struct_name<'a, S> {
                    #[inline]
                    fn #getter_name(&mut self, key: #key_ty) -> #value_ty {
                    ...
                    #[inline]
                    fn #setter_name(&mut self, key: #key_ty, value: #value_ty) {
